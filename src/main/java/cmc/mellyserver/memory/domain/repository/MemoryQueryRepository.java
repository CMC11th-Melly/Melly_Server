...
public class MemoryQueryRepository {
    ...
    public Slice<MemoryResponseDto> searchMemoryUserCreatedForMyPage(Pageable pageable, Long userSeq, GroupType groupType) {
        ...
        List<OrderSpecifier> ORDERS = getAllOrderSpecifiers(pageable); // 정렬 조건

        List<MemoryResponseDto> result = query.select(Projections.constructor(MemoryResponseDto.class, place.id, place.placeName, memory.id, memory.title, memory.content, memory.groupInfo.groupType, memory.groupInfo.groupName, memory.stars, memory.userId.eq(userSeq),memory.visitedDate))
                .from(memory)
                .leftJoin(place).on(place.id.eq(memory.placeId))
                .where(
                        memory.userId.eq(userSeq), // 내가 작성자인 메모리
        ...
        Map<Long, List<ImageDto>> memoryImageList = findMemoryImage(toMemoryIds(result)); // 메모리 사진
        Map<Long, List<KeywordResponse>> keywordList = findKeywordList(toMemoryIds(result)); // 키워드 모음
    ...
    public MemoryResponseDto getMemoryByMemoryId(Long userSeq, Long memoryId) {
        ...
        MemoryResponseDto results = query.select(Projections.constructor(MemoryResponseDto.class, place.id, place.placeName, memory.id, memory.title, memory.content, memory.groupInfo.groupType, memory.groupInfo.groupName, memory.stars,memory.userId.eq(userSeq), memory.visitedDate))
                .from(memory)
                .leftJoin(place).on(place.id.eq(memory.placeId))
                .where(
        ...
        Map<Long, List<ImageDto>> memoryImageList = findMemoryImage(toMemoryIds(List.of(results))); // 메모리 사진
        Map<Long, List<KeywordResponse>> keywordList = findKeywordList(toMemoryIds(List.of(results))); // 키워드 모음

        List.of(results).forEach(f -> {
            f.setMemoryImages(memoryImageList.get(f.getMemoryId()));
            f.setKeyword(keywordList.get(f.getMemoryId()).stream().map(KeywordResponse::getKeyword).collect(Collectors.toList()));
        });
     ...
     * TODO : 찐막 수정 완료
     */
    public Slice<MemoryResponseDto> searchMemoryOtherCreate(Pageable pageable, Long userSeq, Long placeId,GroupType groupType) {

        List<OrderSpecifier> ORDERS = getAllOrderSpecifiers(pageable);

        List<MemoryResponseDto> results = query.select(Projections.constructor(MemoryResponseDto.class, place.id, place.placeName, memory.id, memory.title, memory.content, memory.groupInfo.groupType, memory.groupInfo.groupName, memory.stars, memory.userId.eq(userSeq),memory.visitedDate)) //  다른 사람이 작성한 메모리 가져오기
                .from(memory)
                .leftJoin(place).on(place.id.eq(memory.placeId))
                .where(
...
//                        checkMemoryNotBlocked(userBlockedMemoryId), // 다른 사람이 작성했더라도, 내가 차단한 메모리는 안보이게 하기
        ...
        Map<Long, List<ImageDto>> memoryImageList = findMemoryImage(toMemoryIds(results)); // 메모리 사진
        Map<Long, List<KeywordResponse>> keywordList = findKeywordList(toMemoryIds(results)); // 키워드 모음
