...
public class TrendController {
    ...
    @GetMapping("/trend")
    public  ResponseEntity<CommonResponse<TrendResponseWrapper>> trendPlace(@AuthenticationPrincipal User user)
    {
        List<TrendResponseDto> trend = trendService.getTrend();
        return ResponseEntity.ok(new CommonResponse(200,"핫한 장소",new TrendResponseWrapper(trend)));
    }
}
