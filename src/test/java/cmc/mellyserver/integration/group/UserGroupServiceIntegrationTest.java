package cmc.mellyserver.integration.group;
...
public class UserGroupServiceIntegrationTest extends IntegrationTest {
    ...
    void create_group()
    {
        ...
        CreateGroupRequestDto createGroupRequestDto = CreateGroupRequestDto.builder().userSeq(savedUser.getUserSeq()).groupType(GroupType.FRIEND).groupName("테스트 그룹").groupIcon(1).build();
    ...
    class When_get_group_info_by_identifier{
        ...
        {
            // given
            UserGroup savedGroup = groupRepository.save(UserGroupFactory.mockUserGroup());
    ...
    void remove_group()
    {
        // given
        User savedUser = userRepository.save(UserFactory.createEmailLoginUser());
        UserGroup savedGroup = groupService.saveGroup(CreateGroupRequestDto.builder().userSeq(savedUser.getUserSeq()).groupIcon(1).groupName("테스트 그룹").groupType(GroupType.FRIEND).build());
