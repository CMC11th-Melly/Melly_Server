package cmc.mellyserver.unit.group.application;

import cmc.mellyserver.common.enums.GroupType;
import cmc.mellyserver.common.exception.ExceptionCodeAndDetails;
import cmc.mellyserver.common.exception.GlobalBadRequestException;
import cmc.mellyserver.common.factory.GroupAndUserFactory;
...
public class UserGroupServiceTest{
    ...
    class When_participate_group{
        ...
        {
            // given
            given(authenticatedUserChecker.checkAuthenticatedUserExist(anyLong())).willReturn(UserFactory.createEmailLoginUser());
            given(groupRepository.findById(anyLong())).willReturn(Optional.empty());
        ...
        void participate_group_duplicated_group_participate()
        {
            ...
            given(groupAndUserRepository.findGroupAndUserByUserAndGroup(any(User.class),any(UserGroup.class))).willReturn(Optional.of(GroupAndUserFactory.mockGroupAndUser()));
            // then
            assertThatCode(() -> groupService.participateToGroup(1L,1L))
                    .isInstanceOf(GlobalBadRequestException.class)
            ...
            verify(authenticatedUserChecker,times(1)).checkAuthenticatedUserExist(anyLong());
            verify(groupRepository,times(1)).findById(anyLong());
            verify(groupAndUserRepository,times(1)).findGroupAndUserByUserAndGroup(any(User.class),any(UserGroup.class));
        }
        ...
        void participate_group()
        {
            ...
            given(authenticatedUserChecker.checkAuthenticatedUserExist(anyLong())).willReturn(UserFactory.createEmailLoginUser());
            given(groupRepository.findById(anyLong())).willReturn(Optional.of(UserGroupFactory.mockUserGroup()));
            given(groupAndUserRepository.findGroupAndUserByUserAndGroup(any(User.class),any(UserGroup.class))).willReturn(Optional.empty());
            given(groupAndUserRepository.save(any(GroupAndUser.class))).willReturn(GroupAndUserFactory.mockGroupAndUser());
    ...
    class When_update_group{
        ...
        void update_group_not_exist_exception()
        {
            ...
            // then
            assertThatCode(() ->groupService.updateGroup(updateGroupRequestDto))
                    .isInstanceOf(GlobalBadRequestException.class)
                    .hasMessage(ExceptionCodeAndDetails.NO_SUCH_GROUP.getMessage());
    ...
    class When_remove_group{
        ...
        {
            // given
            UserGroup group = new UserGroup(1L, "동기", 1, "http://naver.com", 2L, GroupType.FRIEND, false);
            given(groupRepository.findById(anyLong())).willReturn(Optional.of(group));

            // then
            assertThatCode(() ->groupService.removeGroup(1L,1L))
                    .isInstanceOf(GlobalBadRequestException.class)
                    .hasMessage(ExceptionCodeAndDetails.NO_AUTHORITY_TO_REMOVE.getMessage());
        ...
        {
            // given
            UserGroup group = new UserGroup(1L, "동기", 1, "http://naver.com", 1L, GroupType.FRIEND, false);
            given(groupRepository.findById(anyLong())).willReturn(Optional.of(group));
