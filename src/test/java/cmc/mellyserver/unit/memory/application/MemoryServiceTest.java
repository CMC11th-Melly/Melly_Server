...
public class MemoryServiceTest {
    ...
    void get_group_list_for_add_memory(){
        ...
        // given
        GroupListForSaveMemoryResponseDto groupListForSaveMemoryResponseDto1 = GroupListForSaveMemoryResponseDto.builder().groupName("그룹 1").groupType(GroupType.FRIEND).build();
        GroupListForSaveMemoryResponseDto groupListForSaveMemoryResponseDto2 = GroupListForSaveMemoryResponseDto.builder().groupName("그룹 2").groupType(GroupType.FAMILY).build();
        GroupListForSaveMemoryResponseDto groupListForSaveMemoryResponseDto3 = GroupListForSaveMemoryResponseDto.builder().groupName("그룹 3").groupType(GroupType.COMPANY).build();
        ...
        // when
        List<GroupListForSaveMemoryResponseDto> groupListLoginUserParticipate = memoryService.findGroupListLoginUserParticipate(1L);
    ...
    void find_memory_by_memoryId(){
        ...
        // given
        MemoryResponseDto memoryResponseDto = MemoryResponseDto.builder().title("메모리 제목").content("메모리 컨텐츠").build();
        given(memoryQueryRepository.getMemoryByMemoryId(anyLong(),anyLong())).willReturn(memoryResponseDto);
    ...
    class When_remove_memory{
      ...
      void memory_not_exist_exception()
      {
        ...
        {
            // given
            Memory memory = Memory.builder().title("메모리 제목").content("메모리 컨텐츠").isDelete(false).build();

            given(memoryRepository.findById(anyLong())).willReturn(Optional.of(memory));

            // when
            memoryService.removeMemory(1L);
