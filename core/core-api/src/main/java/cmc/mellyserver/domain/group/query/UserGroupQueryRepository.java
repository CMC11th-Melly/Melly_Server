...
public class UserGroupQueryRepository {
	...
	public Slice<GroupLoginUserParticipatedResponseDto> getGroupListLoginUserParticipate(Long userId, Long lastId,
		...
		List<GroupLoginUserParticipatedResponseDto> results = query
			.select(Projections.constructor(GroupLoginUserParticipatedResponseDto.class, userGroup.id,
					userGroup.groupIcon, userGroup.groupName, userGroup.groupType))
			.from(groupAndUser)
			.innerJoin(groupAndUser.group, userGroup)
			.innerJoin(groupAndUser.user, user)
			.where(ltGroupId(lastId), eqUser(userId)
