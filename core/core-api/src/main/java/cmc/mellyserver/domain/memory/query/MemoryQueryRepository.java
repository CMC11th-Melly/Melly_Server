...
public class MemoryQueryRepository {
	...
	public Slice<MemoryResponseDto> searchMemoryUserCreatedForPlace(Long lastId, Pageable pageable, Long userId,
		...
		List<MemoryResponseDto> results = query
			.select(Projections.constructor(MemoryResponseDto.class, memory.id, memory.title, memory.visitedDate,
					userGroup.groupType))
			.from(memory)
			.innerJoin(memoryImage)
			.on(memoryImage.memory.id.eq(memory.id))
			.innerJoin(place)
			...
			.fetchJoin()
			.where(isActive(), // =
					ltMemoryId(lastId), createdByLoginUser(userId), // =
					eqPlace(placeId), // =
					eqGroup(groupType) // =
			)
	...
	public Slice<MemoryResponseDto> getMyGroupMemory(Long lastId, Pageable pageable, Long groupId,
			GroupType groupType) {
		...
		List<MemoryResponseDto> results = query
			...
			.on(userGroup.id.eq(memory.groupId))
			.fetchJoin()
			.where(isActive(), ltMemoryId(lastId), memory.groupId.eq(groupId), eqGroup(groupType),
	...
	public Slice<MemoryResponseDto> searchMemoryUserCreatedForMyPage(Long lastId, Pageable pageable, Long userId,
			GroupType groupType) {
		...
		List<MemoryResponseDto> result = query
			...
			.on(userGroup.id.eq(memory.groupId))
			.fetchJoin()
			.where(ltMemoryId(lastId), createdByLoginUser(userId), eqGroup(groupType))
			.limit(pageable.getPageSize() + 1)
			.fetch();
	...
	public MemoryDetailResponseDto findMemoryDetail(Long memoryId) {
		...
		return query.select(Projections.constructor(MemoryDetailResponseDto.class, place.id, // 장소
																								// ID
	...
	public Slice<MemoryResponseDto> searchMemoryOtherCreate(Long lastId, Pageable pageable, Long userId, Long placeId,
			GroupType groupType) {
		...
		List<MemoryResponseDto> result = query
			...
			.on(userGroup.id.eq(memory.groupId))
			.fetchJoin()
			.where(isActive(), ltMemoryId(lastId), eqPlace(placeId), createdByNotCurrentLoginUser(userId), // 내가
																											// 작성자인
																											// 메모리
	...
	public Slice<MemoryResponseDto> getMyGroupMemoryInPlace(Long lastId, Pageable pageable, Long userId, Long placeId,
			GroupType groupType) {

		List<MemoryResponseDto> result = query
			.select(Projections.constructor(MemoryResponseDto.class, memory.id, memory.title, memoryImage.imagePath,
			...
			.on(userGroup.id.eq(memory.groupId))
			.fetchJoin()
			.where(isActive(), ltMemoryId(lastId), inSameGroup(userId), eqPlace(placeId),
					createdByNotCurrentLoginUser(userId), eqGroup(groupType), memory.openType.ne(OpenType.PRIVATE))
			.orderBy(memory.id.desc())
			.limit(pageable.getPageSize() + 1)
			.fetch();
	...
	private static BooleanExpression inSameGroup(Long userId) {
		return memory.userId.in(JPAExpressions.select(groupAndUser.user.id)
			.from(groupAndUser)
			.where(groupAndUser.group.id.in(JPAExpressions.select(groupAndUser.group.id)
				.from(groupAndUser)
