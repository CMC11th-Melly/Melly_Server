...
public class GroupDetailResponseDto implements Serializable {
	...
	public GroupDetailResponseDto(Long groupId, int groupIcon, String groupName, GroupType groupType,
			String invitationLink, List<UserDto> users) {
	...
	public static GroupDetailResponseDto of(UserGroup userGroup, List<User> userList) {
		return new GroupDetailResponseDto(userGroup.getId(), userGroup.getGroupIcon(), userGroup.getGroupName(),
				userList.stream()
					.map(user -> new UserDto(user.getId(), user.getProfileImage(), user.getNickname()))
					.collect(Collectors.toList()),
				userGroup.getGroupType(), userGroup.getInviteLink());
	}
