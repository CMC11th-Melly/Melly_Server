...
public class UserControllerTest extends ControllerTest {
    ...
    void 회원의_프로필정보를_조회한다() throws Exception {
        ...
        // given
        given(userProfileService.getProfile(anyLong())).willReturn(ProfileResponseDto.of(1L, 모카().getNickname(), 모카().getEmail(), 모카().getProfileImage()));
    ...
    @Test
    void 회원의_프로필_이미지를_수정한다() throws Exception {

        // given
        MockMultipartFile mockFile = new MockMultipartFile("mock", "mock".getBytes());

        // when & then
        mockMvc.perform(multipart("/api/users/my-profile/profile-image")
                        .file(mockFile)
                ).andDo(print())
