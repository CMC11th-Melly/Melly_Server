...
public class UserGroupServiceIntegrationTest extends IntegrationTest {
	...
	void 분산락을_적용해_동시접속자가_생겨도_10명제한을_유지한다() throws InterruptedException {
		...
		Thread thread1 = new Thread(() -> groupService.participateToGroup(동시접속유저_1.getId(), 테스트_그룹.getId()));
		Thread thread2 = new Thread(() -> groupService.participateToGroup(동시접속유저_2.getId(), 테스트_그룹.getId()));
		try {
			thread1.start();
			thread2.start();
		}
