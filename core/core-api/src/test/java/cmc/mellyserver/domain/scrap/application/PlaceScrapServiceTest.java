package cmc.mellyserver.domain.scrap.application;
...
public class PlaceScrapServiceTest extends IntegrationTest {
	...
	//
	// CreatePlaceScrapRequestDto createPlaceScrapRequestDto = new
	// CreatePlaceScrapRequestDto(savedUser.getId(),
	// 1.234, 1.234, ScrapType.FRIEND, "스타벅스", "카페");
	// placeScrapService.createScrap(createPlaceScrapRequestDto);
	//
	// // when
	// Slice<ScrapedPlaceResponseDto> scrapedPlace =
	// placeScrapService.findScrapedPlace(PageRequest.of(0, 10),
	...
	//
	// // then
	// Optional<Place> place = placeRepository.findPlaceByPosition(new Position(1.234,
	// 1.234));
	// Optional<PlaceScrap> placeScrap =
	// placeScrapRepository.findByUseridAndPlaceId(savedUser.getId(),
	// place.get().getId());
	...
	//
	// // then
	// assertThatCode(() -> placeScrapService.removeScrap(savedUser.getId(), 1.234,
	// 1.234))
	// .isInstanceOf(GlobalBadRequestException.class)
