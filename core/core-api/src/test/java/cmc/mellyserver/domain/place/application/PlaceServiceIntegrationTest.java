package cmc.mellyserver.domain.place.application;
...
public class PlaceServiceIntegrationTest extends IntegrationTest {
	...
	//
	// Memory memory =
	// Memory.cmc.mellyserver.mellycore.builder().userId(user.getId()).title("테스트
	// 제목").content("테스트 컨텐츠")
	// .groupInfo(new GroupInfo("테스트 그룹", GroupType.FRIEND, 1L))
	// .openType(OpenType.ALL).stars(4L).visitedDate(LocalDateTime.of(2023, 5, 29, 10,
	// 20))
	// .build();
	//
	...
	//
	// // when
	// List<MarkedPlaceResponseDto> markedPlaceResponseDtos =
	// placeService.displayMarkedPlace(user.getId(), null);
	//
	// // then
	...
	//
	// memoryService.createMemory(createMemoryRequestDto);
	// PlaceResponseDto result = placeService.findPlaceByPosition(user.getId(), 1.234,
	// 1.234);
	//
	// // when
	// PlaceResponseDto placeByPlaceId = placeService.findPlaceByPlaceId(user.getId(),
	// result.getPlaceId());
	//
	// // then
	...
	// User user = userRepository.save(UserFactory.createEmailLoginUser());
	//
	// CreatePlaceScrapRequestDto createPlaceScrapRequestDto =
	// CreatePlaceScrapRequestDto.cmc.mellyserver.mellycore.builder()
	// .lng(1.5)
	// .lat(1.5)
	// .placeName("테스트 장소")
	// .id(user.getId())
	...
	// Memory memory =
	// Memory.cmc.mellyserver.mellycore.builder().userId(user.getId()).title("테스트
	// 제목").content("테스트 컨텐츠").build();
	// Memory savedMemory = memoryRepository.save(memory);
	// // when
	// MemoryUpdateFormResponseDto result =
	// memoryService.findMemoryUpdateFormData(user.getId(),
	// savedMemory.getId());
	//
	// // then
