...
public class PlaceQueryRepositoryTest extends RepositoryTest {
	...
	//
	// // given
	// User user = User.cmc.mellyserver.mellycore.builder().nickname("테스트
	// 유저").provider(Provider.DEFAULT).build();
	// User otherUser = User.cmc.mellyserver.mellycore.builder().nickname("테스트
	// 유저2").provider(Provider.DEFAULT).build();
	// User savedUser = userRepository.save(user);
	// User savedOtherUser = userRepository.save(otherUser);
	//
	// Place place = Place.cmc.mellyserver.mellycore.builder().position(new
	// Position(1.234, 1.234)).placeName("테스트 장소")
	// .build();
	// Place savedPlace = placeRepository.save(place);
	//
	// Place place2 = Place.cmc.mellyserver.mellycore.builder().position(new
	// Position(1.235, 1.235)).placeName("테스트 장소2")
	// .build();
	// Place savedPlace2 = placeRepository.save(place2);
	//
	// Memory memory =
	// Memory.cmc.mellyserver.mellycore.builder().userId(savedUser.getId()).placeId(savedPlace.getId())
	// .title("테스트 메모리").build();
	//
	// Memory savedMemory = memoryRepository.save(memory);
	//
	// Memory memory2 =
	// Memory.cmc.mellyserver.mellycore.builder().userId(savedOtherUser.getId())
	...
	// Place savedPlace2 = placeRepository.save(place2);
	//
	// Place place3 = Place.cmc.mellyserver.mellycore.builder().position(new
	// Position(1.236, 1.236)).placeName("테스트 장소3")
	// .build();
	// Place savedPlace3 = placeRepository.save(place3);
	//
	// Place place4 = Place.cmc.mellyserver.mellycore.builder().position(new
	// Position(1.237, 1.237)).placeName("테스트 장소4")
	// .build();
	...
	//
	// PlaceScrap scrap1 = PlaceScrap.createScrap(savedUser, savedPlace2,
	// ScrapType.FRIEND);
	//
	// PlaceScrap scrap2 = PlaceScrap.createScrap(savedUser, savedPlace3,
	// ScrapType.COMPANY);
	//
	// PlaceScrap scrap3 = PlaceScrap.createScrap(savedUser, savedPlace4,
	// ScrapType.COUPLE);
	//
	// placeScrapRepository.saveAll(List.of(scrap, scrap1, scrap2, scrap3));
	//
	// // when
	// List<PlaceScrapCountResponseDto> scrapedPlaceGrouping =
	// placeQueryRepository.getScrapedPlaceGrouping(
	// savedUser);
	//
	// // then
