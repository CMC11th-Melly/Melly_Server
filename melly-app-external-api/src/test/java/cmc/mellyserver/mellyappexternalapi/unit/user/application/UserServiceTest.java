package cmc.mellyserver.mellyappexternalapi.unit.user.application;

import cmc.mellyserver.mellyappexternalapi.common.auth.AuthenticatedUserChecker;
import cmc.mellyserver.mellyappexternalapi.common.aws.impl.AwsServiceImpl;
import cmc.mellyserver.mellyappexternalapi.common.aws.impl.S3FileLoader;
...
import cmc.mellyserver.mellyappexternalapi.user.application.impl.UserServiceImpl;
...
public class UserServiceTest {
    ...
    void find_group_list_login_user_participated() {
        ...
        List<GroupLoginUserParticipatedResponseDto> groupLoginUserParticipatedResponseDtos = List.of(
                new GroupLoginUserParticipatedResponseDto(1L, 1, "친구",
                        List.of(new UserDto(1L, "testImage.png", "jemin", true),
                                new UserDto(2L, "testImage.png", "jemin2", false)),
                        GroupType.FRIEND, "link"),
                new GroupLoginUserParticipatedResponseDto(2L, 2, "동료",
                        List.of(new UserDto(1L, "testImage.png", "jemin", false),
                                new UserDto(2L, "testImage.png", "jemin2", true)),
                        GroupType.COMPANY, "link")
        );
        ...
        // when
        List<GroupLoginUserParticipatedResponseDto> groupListLoginUserParticiated = userService.findGroupListLoginUserParticiated(
                anyLong());
    ...
    void findMemoriesLoginUserWrite() {
        ...
        given(memoryQueryRepository.searchMemoryUserCreatedForMyPage(any(Pageable.class), anyLong(),
                any(GroupType.class)))
                .willReturn(memoryResponseDtos);

        // when
        Slice<MemoryResponseDto> memoriesLoginUserWrite = userService.findMemoriesLoginUserWrite(
                PageRequest.of(0, 2),
                1L, GroupType.FRIEND);
        ...
        verify(memoryQueryRepository, times(1))
                .searchMemoryUserCreatedForMyPage(any(Pageable.class), anyLong(),
                        any(GroupType.class));
    }
    ...
    void findMemoriesUsersBelongToMyGroupWrite() {
        ...
        // when
        Slice<MemoryResponseDto> memoriesUsersBelongToMyGroupWrite = userService.findMemoriesUsersBelongToMyGroupWrite(
                PageRequest.of(0, 2), 1L, 1L);
    ...
    void find_user_profile_data_for_update() {
        ...
        // when
        ProfileUpdateFormResponseDto loginUserProfileDataForUpdate = userService.getLoginUserProfileDataForUpdate(
                anyLong());

        // then
        assertThat(loginUserProfileDataForUpdate.getNickname()).isEqualTo(
                emailLoginUser.getNickname());
        assertThat(loginUserProfileDataForUpdate.getProfileImage()).isEqualTo(
                emailLoginUser.getProfileImage());
        assertThat(loginUserProfileDataForUpdate.getGender()).isEqualTo(emailLoginUser.getGender());
        assertThat(loginUserProfileDataForUpdate.getAgeGroup()).isEqualTo(
                emailLoginUser.getAgeGroup());
    ...
    class When_Signup {
        ...
        void getSurveyResult_Reccommend_Place() {
            ...
            // given
            User emailLoginUser = UserFactory.createEmailLoginUser();
            emailLoginUser.addSurveyData(RecommendGroup.FRIEND, RecommendPlace.PLACE1,
                    RecommendActivity.CAFE);
            ...
            given(surveyRecommender.getRecommend(any(RecommendGroup.class)))
                    .willReturn(surveyRecommendResponseDto);

            // when
            SurveyRecommendResponseDto surveyResult = userService.getSurveyResult(1L);
            ...
            verify(surveyRecommender, times(1))
                    .getRecommend(any(RecommendGroup.class));
        }
    }
