...
public class MemoryServiceIntegrationTest extends IntegrationTest {
	...
	class When_remove_memory {
		...
		void delete_memory() {
			...
			Memory memory = Memory.builder().title("테스트 제목").content("테스트 컨텐츠").userId(user.getUserSeq()).build();
			Memory savedMemory = memoryRepository.save(memory);

			// when
			memoryService.removeMemory(savedMemory.getId());
		...
		void memory_not_exist_exception() {
			...
			// when then
			assertThatThrownBy(() -> memoryService.removeMemory(10L))
	...
	void get_place_info_by_memory_name() {
		...
		PlaceResponseDto placeByPosition = placeService.findPlaceByPosition(user.getUserSeq(), 1.234, 1.234);
	...
	class When_update_memory {
		...
		void update_memory() throws IOException {
			...
			MockMultipartFile mockMultipartFile = new MockMultipartFile("testImage", "test", "multipart/form-data",
				new FileInputStream(
			...
			// then
			Memory findMemory = memoryRepository.findById(updateMemoryRequestDto.getMemoryId()).get();
	...
	class When_find_memories_in_place {
		...
		void get_login_user_write_memories() {
			...
			PlaceResponseDto place = placeService.findPlaceByPosition(user.getUserSeq(), 1.234, 1.234);
			...
			assertThat(loginUserWriteMemoryBelongToPlace.getContent()).extracting("title", "content")
				.containsExactlyInAnyOrder(Tuple.tuple("테스트 제목1", "테스트 컨텐츠1"), Tuple.tuple("테스트 제목2", "테스트 컨텐츠2"));
		}
		...
		void get_other_user_write_memories() {
			...
			User nonLoginUser = userRepository.save(
				User.builder().nickname("test_user2").password("1234").ageGroup(AgeGroup.TWO).build());
