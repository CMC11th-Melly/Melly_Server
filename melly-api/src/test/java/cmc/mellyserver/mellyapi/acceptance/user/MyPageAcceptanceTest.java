...
import static cmc.mellyserver.mellyapi.acceptance.fixtures.UserAcceptanceFixtures.*;
...
public class MyPageAcceptanceTest extends AcceptanceTest {
    ...
    void 로그인한_사용자는_마이페이지에서_자신이_속한_그룹을_조회할수있다() {
        ...
        ApiResponse<List<GroupLoginUserParticipatedResponse>> result = response.as(
                new TypeRef<ApiResponse<List<GroupLoginUserParticipatedResponse>>>() {
                });
    ...
    void 로그인한_사용자는_마이페이지에서_자신의_프로필을_수정할수있다() {
        ...
        ApiResponse<ProfileUpdateFormResponseDto> result = response.as(
                new TypeRef<ApiResponse<ProfileUpdateFormResponseDto>>() {
                });
