...
public class MemoryControllerTest extends ControllerTest {
	...
	void get_memories_login_user_write_in_place() throws Exception {
		...
		SliceImpl<MemoryResponseDto> slice = new SliceImpl<>(
			List.of(memoryResponseDto1, memoryResponseDto2, memoryResponseDto3), PageRequest.of(0, 10), true);

		given(memoryService.findLoginUserWriteMemoryBelongToPlace(any(Pageable.class), anyLong(), anyLong(),
			any(GroupType.class)))
			.willReturn(slice);
	...
	void get_memories_other_user_write_in_place() throws Exception {
		...
		given(memoryService.findOtherUserWriteMemoryBelongToPlace(any(Pageable.class), anyLong(), anyLong(),
			any(GroupType.class)))
			.willReturn(slice);
	...
	void get_memories_my_group_user_write_in_place() throws Exception {
		...
		given(memoryService.findMyGroupMemberWriteMemoryBelongToPlace(any(Pageable.class), anyLong(), anyLong(),
			any(GroupType.class)))
			.willReturn(slice);
	...
	void saveMemory() throws Exception {
		...
		// given
		MockMultipartFile mockFile1 = new MockMultipartFile("content", "filename", "multipart/form-data",
			new FileInputStream(
				"/Users/seojemin/IdeaProjects/Melly_Server/melly-api/src/test/java/resources/image/testimage.png"));
		MockMultipartFile mockFile2 = new MockMultipartFile("content", "filename", "multipart/form-data",
			new FileInputStream(
				"/Users/seojemin/IdeaProjects/Melly_Server/melly-api/src/test/java/resources/image/testimage.png"));
	...
	void get_place_contains_memory_name() throws Exception {
		...
		FindPlaceInfoByMemoryNameResponseDto findPlaceInfoByMemoryNameResponseDto3 = FindPlaceInfoByMemoryNameResponseDto.builder()
			...
			.memoryName("테스트")
			.build();
		List<FindPlaceInfoByMemoryNameResponseDto> list = List.of(findPlaceInfoByMemoryNameResponseDto1,
